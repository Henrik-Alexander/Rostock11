...
# set theme
theme_set(theme_classic(base_size = 18, base_family = "Roboto"))
theme_update(plot.margin = margin(0.1, 0.6, 0.1, 0.1, "cm"),
             panel.grid.major.x = element_blank(),
             panel.grid.minor.x = element_blank(),
             panel.grid.minor.y = element_blank(),
             legend.title = element_text(face = "bold"),
             axis.title.x = element_text(family = "Roboto Light", size = 14, hjust = 1),
             axis.title.y = element_text(family = "Roboto Light", size = 14, hjust = 1, angle = 0),
...
```{r Mikko-plot0, fig.height=5, fig.width=4}
setwd("U:/Rostock11")
# Load the data
...
```{r Mikko-plot, fig.height=5, fig.width=4}
# Plot mikko's graph
 d %>% filter(Year == 1975) %>% 
...
---
## Meine Motivation
```{r Mikko-plot2, fig.height=5, fig.width=4}
# Plot mikko's graph
 d %>% 
  ggplot(aes(HDI_trans, TFR, colour = Year, shape = Year)) +
...
ggplot(subset(df, Division == "Midwest"), aes(lag_HLI_female, TFR_female, colour = Year)) +
  ...
  ylab("Allgemeine \n Geburtenziffer") + xlab("Entwicklungsindikator") +
...
# Plot
ggplot(subset(d, Year >= 1965), aes(Year, TFR)) +
  geom_point(colour = MPIDRpurple) +
  geom_smooth(method = "lm", se = F, formula = "y ~ poly(x, 2)", linewidth = 2, colour = MPIDRgreen) +
  ylab("Zusammengefasste \n Fruchtbarkeitsziffer") + xlab("Jahr") +
  scale_colour_gradient(low = MPIDRpurple, high = MPIDRgreen, name = "Jahr") +
  guides(colour = "none") +
